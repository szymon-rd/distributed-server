 ...
 */
case class SendMessage(i: Short, l: Short, m: Array[Byte], s: Session) extends InPacket(i, l, m, s) {
  implicit val charset = Chat.charset
  private val nameLength = m.readShort()
  val channelName = m.readString(nameLength)
  private val messageLength = m.readShort()
  val message = m.readString(messageLength)
}
